...
class ScoreGuidedAutoencoderNetwork(nn.Module):
    def __init__(self, input_shape, hidden_dim_input, layer_list=[20, 40, 80, 256, 1024], seed=None):
        '''
        hidden_dim_input: str
        '''
        super(ScoreGuidedAutoencoderNetwork, self).__init__()
    ...
    def loss_function(self, x, x_dec, scores, anomal_flag, lam_dist, a, lam_out, device):
        '''Reoconstruction error'''
        recon_error = torch.mean((x - x_dec) ** 2)
        dist_error = self.compute_dist_error(scores, anomal_flag, a, lam_out, device)
        loss = recon_error + lam_dist * dist_error

        return loss, recon_error, dist_error

    def compute_dist_error(self, scores, anomal_flag, a, lam_out, device):
        ...
        outlier_loss = torch.abs(torch.max(a - scores, torch.zeros(scores.shape).to(device)))
        dist_error = torch.mean((1 - anomal_flag) * inlier_loss + lam_out * anomal_flag * outlier_loss)
